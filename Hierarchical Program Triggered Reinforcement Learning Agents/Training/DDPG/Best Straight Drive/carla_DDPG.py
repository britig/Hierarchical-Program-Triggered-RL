	...
	for epi in range(episodes):
		try:
			...
			for i in range(max_step):
				...
				radar_state_next, next_state, reward, done, length_traversed = agent.step_straight(action, 1)
				time.sleep(0.5)
			...
			straight_model.tensorboard.update_stats(reward_avg=[None, avg_reward], critic_loss=[None, np.mean(critic_loss_epi)], actor_loss=[None, np.mean(actor_loss_epi)], lenght_covered=[None, np.mean(avg_length)])
