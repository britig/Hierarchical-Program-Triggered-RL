...
class CarlaVehicle(object):
	...
	def reset(self,Norender):
		...
		init_pos = carla.Transform(carla.Location(x = 135, y = 130, z=2), carla.Rotation(yaw=180))
	...
	def step_straight(self, action, p):
		...
		data = np.array(self.radar_data)
		length_traversed = abs(current_location.x - 135)
		return data[-LIMIT_RADAR:], [round(kmh, ROUNDING_FACTOR)] , reward, done, length_traversed
