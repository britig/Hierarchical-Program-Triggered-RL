...
def is_within_distance_ahead(target_transform, current_transform, max_distance):
    ...
    #print(f'norm target from is within========{norm_target}')
...
def target_magnitude(ego_location,target_location):
    target_vector = np.array([float(target_location[0]) - ego_location[0], float(target_location[1]) - ego_location[1]])
    norm_target = np.linalg.norm(target_vector)
    return norm_target
