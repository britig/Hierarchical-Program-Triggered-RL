...
"""
Welcome to CARLA neural network control.
...
from srunner.tools.scenario_route_parser import ScenarioRouteParser
#Utility tool to draw waypoints
...
class World(object):
	def __init__(self, carla_world, hud):
		...
		#adding obstacle sensor for static object detection
		self.obstacle_sensor = ObstacleSensor(self.vehicle, self.hud)
...
class ObstacleSensor(object):
	...
	def _on_obstacle(weak_self, event):
		...
		#print ("Event, in line of sight with %s at distance %u" % (event.other_actor.type_id, event.distance))
...
def game_loop(args):
	...
	try:
		...
		#Get the world map from Carla Client
		world_map = client.get_world().get_map()
		scenario_name = args.scenario
		print(f'The current scenario is ======== {scenario_name}')

		#Creates the option list with start and end waypoints
		robj = ScenarioRouteParser(scenario_name)
		route_list = robj._extract_route_from_tree()
		print(f'route list ============== {route_list}')

		#Create Hierararchical Program Triggered Reinforcement Learning agent
		agent = NNAgent(world.vehicle,world_map,route_list)
		waypoint_list = []
		while True:
			clock.tick_busy_loop(60)
			waypoint_list.append(world_map.get_waypoint(world.vehicle.get_location()))
		...
		#draw_waypoints(client.get_world(),waypoint_list,2)
		time.sleep(10)
...
def main():
	argparser = argparse.ArgumentParser(
		description='CARLA Neural Network Control Client')
	...
	argparser.add_argument('--scenario', help='Name of the scenario for getting route value.')
